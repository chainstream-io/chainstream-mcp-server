...
export class RankingTool {
  @Tool({
    ...
    parameters: z.object({
      chain: z.enum(['sol', 'base', 'bsc', 'polygon', 'arbitrum', 'optimism', 'avalanche', 'ethereum', 'zksync', 'sui']).describe('Chain name'),
      duration: z.enum(['1m', '5m', '1h', '4h', '24h']).describe('Time duration for ranking'),
      ...
      accessToken: z.string().describe('JWT access token for ChainStream API authentication'),
    }),
    annotations: {
  ...
  async getHotTokens({ chain, duration, sortBy, sortDirection, filterBy, accessToken }) {
    try {
